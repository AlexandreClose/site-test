...
//    	        }
    	    	...
    	    	formDisplayUploadedFiles${fieldname}( data.result, data.files, '${deletePrefix}' );
    	...
    	$.getJSON('${base_url}jsp/site/plugins/asynchronousupload/DoRemoveFile.jsp', jsonData,
    			function(json) {
    				formDisplayUploadedFiles${fieldname}(json, null, '${deletePrefix}');
    			}
    		);
    });
    ...
    $(document).on('click','[name^="${deletePrefix}"]', {} ,function() {
		...
		var strIndex = fieldName.slice(-1);
	
		fieldName = fieldName.substring(0, fieldName.length-1);
		
    	removeFile(fieldName, '${handler_name}', '${base_url}', strIndex);
    	event.preventDefault( );
    });
...
function formDisplayUploadedFiles${fieldname}( jsonData, files, cbPrefix )
{
	...
	var handlerName = jsonData.handler_name;
...
//				{
							...
							strContent = strContent  + "<div class=\"form-inline light-margin btn-max-width\" id=\"_file_uploaded_" + fieldName + index + "\"><div class=\"form-group col-xs-11 col-sm-11  no-margin no-padding text-yellow\"><label class=\"col-xs-12 col-sm-12 no-padding text-yellow userstepsloadfiles-file-line\">"
					+ "&bull; " + ( (jsonData.fileCount == 1) ? jsonData.files.name : jsonData.files[index].name ) 	+ "</label></div>" + "<div class=\"form-group col-xs-1 col-sm-1  no-margin no-padding\">" +  
					
					"<button class=\"col-xs-12 col-sm-12 userstepsloadfiles-btn-cancel\" name=\"" + cbPrefix + fieldName + index + "\"" +  " value=\"" + cbPrefix + fieldName + index + "\"" +  "type=\"submit\"" +  "><span class=\"picto-cancel-circled\"></button></div>"
 ...
 */
function removeFile( fieldName, handlerName, baseUrl, strIndex ) {
	// build indexes to remove
	

	var jsonData = {"fieldname":fieldName, "asynchronousupload.handler":handlerName, "field_index": strIndex};
